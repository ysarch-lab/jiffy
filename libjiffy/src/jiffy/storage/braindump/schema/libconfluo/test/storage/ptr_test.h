...
TEST_F(PtrTest, CopyTest) {
  size_t initial_mem_usage = allocator::instance().memory_utilization();

  {
    void *data = allocator::instance().alloc(sizeof(uint64_t) * ARRAY_SIZE, ptr_aux_block());
    encoded_ptr<uint64_t> enc_ptr(data);
    swappable_encoded_ptr<uint64_t> ptr(enc_ptr);

    ASSERT_EQ(initial_mem_usage + ALLOC_SIZE, allocator::instance().memory_utilization());
    {
  ...
  ASSERT_EQ(initial_mem_usage, allocator::instance().memory_utilization());
}
...
TEST_F(PtrTest, SwapTest) {
...

  {
    ...
    void *data = allocator::instance().alloc(sizeof(uint64_t) * ARRAY_SIZE, aux_unarchived);
    void *data_swapped = allocator::instance().alloc(sizeof(uint64_t) * ARRAY_SIZE, aux_archived);
...

    {
      ...
      ASSERT_EQ(data, ptr_copy.get().ptr());
      {
        ...
        ASSERT_EQ(initial_mem_usage + 2 * ALLOC_SIZE, allocator::instance().memory_utilization());
