#ifndef CONFLUO_TEST_SCHEMA_PARSER_TEST_H_
#define CONFLUO_TEST_SCHEMA_PARSER_TEST_H_
...
TEST_F(SchemaParserTest, BasicTest) {
  std::string schema = "{a: BOOL, b: CHAR, c: SHORT, d: INT, e: LONG, f: FLOAT, g: DOUBLE, h: STRING(16)}";
  ...
  ASSERT_TRUE(primitive_types::BOOL_TYPE() == schema_vec[1].type());
  ...
  ASSERT_TRUE(primitive_types::CHAR_TYPE() == schema_vec[2].type());
  ...
  ASSERT_TRUE(primitive_types::SHORT_TYPE() == schema_vec[3].type());
  ...
  ASSERT_TRUE(primitive_types::INT_TYPE() == schema_vec[4].type());
  ...
  ASSERT_TRUE(primitive_types::LONG_TYPE() == schema_vec[5].type());
  ...
  ASSERT_TRUE(primitive_types::FLOAT_TYPE() == schema_vec[6].type());
  ...
  ASSERT_TRUE(primitive_types::DOUBLE_TYPE() == schema_vec[7].type());
  ...
  ASSERT_TRUE(primitive_types::STRING_TYPE(16) == schema_vec[8].type());
  ASSERT_FALSE(schema_vec[8].is_indexed());
}

#endif /* CONFLUO_TEST_SCHEMA_PARSER_TEST_H_ */
